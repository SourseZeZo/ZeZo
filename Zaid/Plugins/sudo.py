...
import sys
import traceback
from ZeZo.MusicUtilities.database.sudo import (get_sudoers, get_sudoers, remove_sudo, add_sudo)
...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            ...
            return os.execvp("python3", ["python3", "-m", "Zaid"])
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if user.id not in await get_sudoers():
            return await message.reply_text(f"❌ Not a part of Sudo.")        
        removed = await remove_sudo(user.id)
        if removed:
            await message.reply_text(f"✅ Removed **{user.mention}** from Sudo.")
            return os.execvp("python3", ["python3", "-m", "Zaid"])
